 ...
 *
 * PATCH (plane-custom) v1.23d hotfix: Gantt drag click leak.
 *  In Gantt, dopo aver trascinato un block, al rilascio del mouse il
 *  browser invia un evento `click` (perche' tecnicamente il pulsante e'
 *  stato rilasciato sopra l'elemento che aveva ricevuto mousedown).
 *  Questo apriva il peek-overview alla fine di ogni drag.
 *
 *  Soluzione: tracciare la posizione del mouse al mousedown e calcolare
 *  la distanza al click. Se > 5px, l'utente ha trascinato -> ignoro
 *  l'apertura del peek. Se <= 5px, era un click "vero" -> apro peek.
 *
 *  Solo `IssueGanttBlock` (block sulla barra Gantt) e' affetto. Il
 *  `IssueGanttSidebarBlock` (sidebar a sinistra) non e' draggable, lo
 *  lascio invariato.
 */
...
// Soglia drag in pixel. Sotto, considero click vero. Sopra, drag.
...
export const IssueGanttBlock = observer(function IssueGanttBlock(props: Props) {
  ...
  // PATCH v1.23d: ref per tracciare la posizione del mouse al mousedown.
  ...
  const handleIssuePeekOverview = (e: React.MouseEvent<HTMLDivElement>) => {
    // Se l'utente ha trascinato (mouse mosso > soglia), NON aprire il peek.
...
// rendering issues on gantt sidebar (non draggable -> lasciato invariato)
