 ...
 *
 * PATCH (plane-custom) v1.07 - Fix getGroupByColumns per Workspace Views.
 *
 * Root cause: getStateColumns / getCreatedByColumns in workspace-level
 * (GLOBAL store, viewId='all-issues', ...) ricevono projectId undefined
 * e ricadono su projectStates / project-level memberIds, che in scope
 * workspace valgono undefined. Ritornano undefined, risale a
 * getGroupByColumns -> List/KanBan default.tsx fanno if (!groups) return
 * null -> schermo BIANCO.
 *
 * Fix:
 *  - getStateColumns: se isWorkspaceLevel e !projectId, usa
 *    store.state.workspaceStates; triggera fetchWorkspaceStates se non
 *    ancora caricati (mobx re-render automatico quando arrivano).
 *  - getCreatedByColumns: fallback a workspaceMemberIds quando
 *    isWorkspaceLevel.
 *  - Route map: created_by ora passa TGetColumns alla sua funzione.
 */
...
export const getGroupByColumns = ({
  ...
  dlog("layouts-utils", "getGroupByColumns called", {
    groupBy,
    includeNone,
    isWorkspaceLevel,
    isEpic,
  ...
  if (!groupBy && includeNone) {
    dlog("layouts-utils", "getGroupByColumns -> All Issues bucket", { includeNone });
    return [
      {
        id: "All Issues",
  ...
  if (!groupBy) {
    dlog("layouts-utils", "getGroupByColumns -> undefined (!groupBy && !includeNone)");
    return undefined;
  }
  ...
  > = {
    ...
    created_by: (args: TGetColumns) => getCreatedByColumns(args),
    team_project: getTeamProjectColumns,
  };

  // Get and return the columns for the specified group by option
  const result = groupByColumnMap[groupBy]?.({ isWorkspaceLevel, projectId });
  dlog("layouts-utils", "getGroupByColumns -> result", {
    groupBy,
    resultType: Array.isArray(result) ? "array" : typeof result,
    resultLength: Array.isArray(result) ? result.length : null,
    firstFewIds: Array.isArray(result)
      ? result.slice(0, 5).map((c) => c.id)
      : null,
  });
  return result;
};
...
const getStateColumns = ({
  ...
  const stateStore = store.state as unknown as {
    getProjectStates: (projectId: string | null | undefined) => any[] | undefined;
    projectStates: any[] | undefined;
    workspaceStates: any[] | undefined;
    fetchWorkspaceStates: (workspaceSlug: string) => Promise<unknown>;
  };
  const { getProjectStates, projectStates, workspaceStates, fetchWorkspaceStates } = stateStore;

  dlog("layouts-utils", "getStateColumns enter", {
    isWorkspaceLevel,
    projectId,
    hasProjectStates: Array.isArray(projectStates) ? projectStates.length : null,
    hasWorkspaceStates: Array.isArray(workspaceStates) ? workspaceStates.length : null,
  });

  // PATCH v1.07: scope-aware source selection.
  let _states: any[] | undefined;
  if (projectId) {
    _states = getProjectStates(projectId);
    dlog("layouts-utils", "getStateColumns using projectStates", {
      projectId,
      count: Array.isArray(_states) ? _states.length : null,
    });
  } else if (isWorkspaceLevel) {
    _states = workspaceStates;
    dlog("layouts-utils", "getStateColumns using workspaceStates", {
      count: Array.isArray(_states) ? _states.length : null,
    });
    // Trigger fetch if not loaded yet. Mobx will re-emit when data lands,
    // so List/KanBan auto re-render out of blank state.
    if (!_states && typeof window !== "undefined") {
      try {
        const workspaceSlug = (store as any)?.router?.workspaceSlug;
        dlog("layouts-utils", "workspaceStates undefined -> fetchWorkspaceStates", {
          workspaceSlug,
          hasFetcher: typeof fetchWorkspaceStates === "function",
        });
        if (workspaceSlug && typeof fetchWorkspaceStates === "function") {
          fetchWorkspaceStates(workspaceSlug).catch((e: unknown) => {
            dlog("layouts-utils", "fetchWorkspaceStates failed", {
              message: (e as Error)?.message,
            });
          });
        }
      } catch (e) {
        dlog("layouts-utils", "fetchWorkspaceStates threw sync", {
          message: (e as Error)?.message,
        });
      }
    }
    ...
    dlog("layouts-utils", "getStateColumns fallback projectStates", {
      count: Array.isArray(_states) ? _states.length : null,
    });
  }

  if (!_states) {
    dlog("layouts-utils", "getStateColumns -> undefined (no states)");
    return;
  }
  return _states.map((state: any) => ({
    id: state.id,
    name: state.name,
    icon: (
...
const getCreatedByColumns = (opts?: TGetColumns): IGroupByColumn[] | undefined => {
  // PATCH v1.07: workspace-level support.
  const memberRootAny = store.memberRoot as any;
  const { getUserDetails } = memberRootAny;

  let memberIds: string[] | undefined;
  if (opts?.isWorkspaceLevel) {
    memberIds =
      memberRootAny?.workspace?.workspaceMemberIds ??
      memberRootAny?.workspaceMemberIds ??
      memberRootAny?.project?.projectMemberIds;
  } else {
    memberIds = memberRootAny?.project?.projectMemberIds;
  }
