 ...
 *  Rimosso il gate `projectId &&` davanti al checkbox di multi-select. Nello
 *  stock il checkbox e' visibile solo in project views (URL con projectId);
 *  in workspace views/your-work era nascosto. Lo abilito anche fuori da
 *  project context. La logica `disabled` si adatta:
 *    - in project context: disabled se l'issue e' di un altro project
 *      (cosi' non si selezionano issue cross-project per sbaglio)
 *    - in workspace context: mai disabled per project mismatch (la barra
 *      gestisce il group-by-project nelle bulk operations)
 */
...
export const IssueBlock = observer(function IssueBlock(props: IssueBlockProps) {
  ...
  // PATCH v1.27a: gate per il checkbox.
  // - In project context (URL ha projectId): tooltip "only same project"
  //   visibile, e checkbox disabled per issue di altri project.
  // - In workspace context (no projectId URL): nessun gate per project,
  //   selezionabile sempre.
  const isCheckboxDisabled = !!projectId && issue.project_id !== projectId;
  const checkboxTooltipDisabled = !projectId || issue.project_id === projectId;
        ...
        <div className="flex w-full gap-2 truncate">
          <div className="flex flex-grow items-center gap-0.5 truncate">
            <div className="flex items-center gap-1" style={isSubIssue ? { marginLeft } : {}}>
              {/* select checkbox - PATCH v1.27a: rimosso `projectId &&` gate */}
              {canSelectIssues && !isEpic && (
                <Tooltip
                  tooltipContent={
                    <>
                  ...
                  disabled={checkboxTooltipDisabled}
                >
