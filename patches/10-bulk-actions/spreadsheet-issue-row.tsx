 ...
 *
 * PATCH (plane-custom) v1.27c:
 *  Stesso fix di v1.27a su list/block.tsx ma per Spreadsheet. Rimosso il
 *  gate `projectId &&` davanti al checkbox di multi-select cosi' la
 *  selezione e' disponibile anche in workspace views/your-work. La logica
 *  disabled si adatta:
 *    - in project context (URL ha projectId): disabled se issue di altro
 *      project
 *    - in workspace context: nessun gate per project mismatch
 */
...
const IssueRowDetails = observer(function IssueRowDetails(props: IssueRowDetailsProps) {
  ...
  // PATCH v1.27c: gate adatto al context (vedi v1.27a su list/block.tsx).
  const isCheckboxDisabled = !!projectId && issueDetail.project_id !== projectId;
  const checkboxTooltipDisabled = !projectId || issueDetail.project_id === projectId;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {/* select checkbox - PATCH v1.27c: rimosso `projectId &&` gate */}
              {canSelectIssues && (
                <Tooltip
                  tooltipContent={
                    <>
