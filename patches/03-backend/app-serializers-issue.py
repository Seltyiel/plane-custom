...
# Django imports
# PATCH (plane-custom) v1.20 hotfix #2:
#   IssueSerializer.validate accetta come state_id valido sia gli state
#   del progetto sia gli workspace shared states (project=NULL del medesimo
#   workspace). Patch applicata in 2 punti del file (il check su state in
#   IssueCreateSerializer e quello su state_id in IssueListDetailSerializer).
...
class IssueCreateSerializer(BaseSerializer):
    ...
    def validate(self, attrs):
        ...
        # PATCH v1.20 hotfix #2b: deriva workspace_id per validare workspace
        # shared states. Lo IssueViewSet passa solo project_id nel context,
        # quindi prendiamo workspace_id da:
        #   1) self.instance.workspace_id (in partial_update / update)
        #   2) Project.workspace_id lookup via project_id (in create)
        #   3) None (improbabile, ma fallback safe = la condizione shared
        #      non matchera' nessun record)
        _ws_id_for_state_validation = None
        if self.instance is not None:
            _ws_id_for_state_validation = getattr(self.instance, "workspace_id", None)
        if _ws_id_for_state_validation is None:
            from plane.db.models import Project as _Project
            _pid = self.context.get("project_id")
            if _pid:
                _ws_id_for_state_validation = (
                    _Project.objects.filter(pk=_pid).values_list("workspace_id", flat=True).first()
                )
        ...
        # PATCH v1.20 hotfix #2: accetta sia state del progetto sia workspace
        # shared state (project=NULL dello stesso workspace).
        # workspace_id derivato sopra (hotfix #2b) per evitare il bug del context
        # mancante in IssueViewSet.partial_update.
        if (
            attrs.get("state")
            and not state_manager.filter(
                Q(project_id=self.context.get("project_id"))
                | Q(project__isnull=True, workspace_id=_ws_id_for_state_validation),
...
class IssueSerializer(DynamicBaseSerializer):
    ...
    def validate(self, data):
        # PATCH v1.20 hotfix #2 (con fallback #2b): deriva workspace_id se
        # mancante dal context (vedi IssueCreateSerializer.validate sopra).
        _ws_id = None
        if self.instance is not None:
            _ws_id = getattr(self.instance, "workspace_id", None)
        if _ws_id is None:
            _ws_id = self.context.get("workspace_id")
        if _ws_id is None:
            from plane.db.models import Project as _Project
            _pid = self.context.get("project_id")
            if _pid:
                _ws_id = _Project.objects.filter(pk=_pid).values_list("workspace_id", flat=True).first()

        if (
            data.get("state_id")
            and not State.objects.filter(
                Q(project_id=self.context.get("project_id"))
                | Q(project__isnull=True, workspace_id=_ws_id),
