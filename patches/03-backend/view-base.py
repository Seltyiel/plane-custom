...
# PATCH v1.15 - backend grouping per workspace views ("all-issues").
#
# STOCK: WorkspaceViewIssuesViewSet.list() paginava senza mai leggere
# group_by/sub_group_by -> rispondeva sempre con `grouped_by: null` e
# `results: array[...]` piatto. Il frontend Kanban workspace chiedeva
# group_by=state__group ma il backend ignorava il parametro; lo store
# bucketava tutto in "All Issues" e la Kanban appariva bianca (colonne
# backlog/unstarted/... tutte vuote con showEmptyGroup=false).
#
# PATCH: portato il pattern da workspace/user.py
# (WorkspaceUserProfileIssuesEndpoint.get) che la Kanban di "Your Work"
# usa con successo:
#   1) legge group_by / sub_group_by dalla query string
#   2) chiama issue_queryset_grouper per annotare il queryset
#   3) nel ramo CON group_by usa GroupedOffsetPaginator /
#      SubGroupedOffsetPaginator + issue_on_results + issue_group_values
#   4) nel ramo SENZA group_by LASCIA invariato il comportamento stock
#      (ViewIssueListSerializer) per non rompere List/Spreadsheet/Gantt
#      che gia' funzionano.
#
# Gli import extra (grouper / paginator) sono additivi, non rimuovono
# nessuno dei simboli usati nel file stock.
...
# PATCH v1.15 - import aggiuntivi per grouping server-side
...
class WorkspaceViewIssuesViewSet(BaseViewSet):
    ...
    def list(self, request, slug):
        ...
        # PATCH v1.15 - grouping server-side per Kanban/Gantt/Calendar workspace.
        # Replica del pattern in WorkspaceUserProfileIssuesEndpoint.get()
        # (workspace/user.py). Se group_by non e' nella query string ricadiamo
        # sul branch stock (ViewIssueListSerializer) per non toccare
        # List/Spreadsheet che gia' funzionano.
        group_by = request.GET.get("group_by", False)
        sub_group_by = request.GET.get("sub_group_by", False)

        if group_by:
            # Annotate queryset con i campi per grouping
        ...
        # Ramo stock (no group_by) - invariato
        return self.paginate(
            order_by=order_by_param,
            request=request,
            queryset=issue_queryset,
