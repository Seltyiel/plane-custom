...
#  Quando un nuovo progetto viene creato, controlla se esistono workspace
#  shared states (State.objects con project=NULL e workspace=questo).
#  Se SI -> NON crea i 6 hardcoded DEFAULT_STATES per il progetto.
#           Il progetto userà gli stati workspace-shared by reference.
#  Se NO -> comportamento stock: bulk_create dei 6 DEFAULT_STATES.
#  Backward-compat: workspace senza shared states (= tutti i workspace
#  esistenti prima di v1.20a) continuano a funzionare come stock.
#  Vedi state.py model patch v1.20a per il modello State con project NULL.
...
class ProjectViewSet(BaseViewSet):
    ...
    def create(self, request, slug):
        ...
        if serializer.is_valid():
            ...
            # PATCH v1.20a: skip default project states if workspace shared
            # states already exist. Nuovi progetti ereditano gli stati
            # workspace-level (riferiti via fetch all_state_objects nello
            # store frontend). Workspace pre-v1.20a senza shared states
            # mantengono il comportamento stock (bulk_create dei 6).
