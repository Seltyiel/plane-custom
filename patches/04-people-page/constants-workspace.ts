 ...
 *  Aggiunta voce "people" al sidebar (Team dashboard frontend).
 *  - Nuova voce in WORKSPACE_SIDEBAR_STATIC_NAVIGATION_ITEMS con key "people",
 *    href "/people/", accessibile a ADMIN e MEMBER (guests esclusi).
 *  - Aggiunta al WORKSPACE_SIDEBAR_STATIC_PINNED_NAVIGATION_ITEMS_LINKS
 *    cosi' viene mostrata come voce sempre visibile sotto projects.
 *  - "people" aggiunto anche agli staticItems in sidebar-item.tsx tramite
 *    patch di helper.tsx (plane-web/sidebar/helper.tsx) per l'icona.
 *  - labelTranslationKey = "People" (stringa letterale): t() ritorna la chiave
 *    stessa quando non la trova nei locale, evitando di dover patchare
 *    ogni file di traduzione (en/it/de/fr/... translations.ts).
 */
...
export const WORKSPACE_SIDEBAR_STATIC_NAVIGATION_ITEMS: Record<string, IWorkspaceSidebarNavigationItem> = {
  ...
  // plane-custom v1.19: Team dashboard - People page.
  // Mostriamo a ADMIN e MEMBER; i Guest non vedono la voce.
  // labelTranslationKey e' impostato alla stringa letterale "People": non
  // esiste nei locale files, quindi t("People") cade nel fallback (store/index.ts
  // riga 244) e restituisce la chiave stessa. Cosi' evitiamo di dover toccare
  // tutti i file en/translations.ts, it/translations.ts ecc. per una singola
  // voce; se in futuro serve la traduzione, basta aggiungere la chiave ai
  // locale files e cambiare qui in "sidebar.people".
  people: {
    key: "people",
...
export const WORKSPACE_SIDEBAR_STATIC_PINNED_NAVIGATION_ITEMS_LINKS: IWorkspaceSidebarNavigationItem[] = [
  ...
  // plane-custom v1.19: People e' fissa sotto Projects, prima delle dynamic.
