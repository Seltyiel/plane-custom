 ...
 *  Aggiunto createMoveMenuItem factory + integrazione in
 *  useProjectIssueMenuItems, useAllIssueMenuItems, useCycleIssueMenuItems,
 *  useModuleIssueMenuItems. Apre il MoveIssueModal via callback dal caller.
 */

import { useMemo } from "react";
import { ArrowRightLeft, XCircle, ArchiveRestoreIcon } from "lucide-react";
// plane imports
...
export interface MenuItemFactoryProps {
  ...
  // PATCH v1.24c: nuovo setter per il MoveIssueModal. Opzionale: i wrapper
  // che non vogliono mostrare la voce "Move to project" possono ometterlo.
  setMoveIssueModalOpen?: (open: boolean) => void;
...
export const useMenuItemFactory = (props: MenuItemFactoryProps) => {
  ...
  // PATCH v1.24c: voce "Move to project". Visibile solo se il caller ha
  // passato setMoveIssueModalOpen (cioe' il wrapper che monta il modal).
  // Si nasconde se l'utente non ha permesso editing.
  const createMoveMenuItem = (): TContextMenuItem => ({
    key: "move-to-project",
    title: "Move to project",
    icon: ArrowRightLeft,
    action: () => {
      if (setMoveIssueModalOpen) setMoveIssueModalOpen(true);
    },
    shouldRender: !!setMoveIssueModalOpen && isEditingAllowed,
  });
