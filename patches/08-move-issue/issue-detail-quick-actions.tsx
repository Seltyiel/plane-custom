 ...
 *
 * PATCH (plane-custom) v1.24c: state + render del MoveIssueModal +
 * pass setMoveIssueModalOpen ai menuItemProps. Helper.tsx
 * (useWorkItemDetailMenuItems) include la voce "Move to project".
 */
...
export const WorkItemDetailQuickActions = observer(function WorkItemDetailQuickActions(
  ...
  const MENU_ITEMS = baseMenuItems
    .map((item) => {
      if (item.key === "edit") {
        return { ...item, shouldRender: isEditingAllowed && !isPeekMode };
      }
      if (item.key === "delete") {
        return { ...item };
      }
      if (item.key === "copy-link") {
        return { ...item, shouldRender: !isPeekMode };
      }
      return item;
    })
  ...
  return (
    <>
      ...
      <MoveIssueModal isOpen={moveIssueModalOpen} onClose={() => setMoveIssueModalOpen(false)} issue={issue} />
