 ...
 *  Espone quickAddIssue in useGlobalIssueActions (workspace views) e
 *  useProfileIssueActions (Your Work). Stock le omette perche' i loro
 *  store types avevano `quickAddIssue: undefined`.
 *
 *  Profile/assigned auto-assign:
 *    Su /profile/<userId>/assigned il filtro server-side e'
 *    assignees=userId, quindi un task quick-creato senza assignee
 *    sparirebbe dalla lista dopo il refresh. Auto-aggiungiamo userId
 *    a assignee_ids prima del POST. Su /profile/<userId>/created il
 *    backend imposta created_by=request.user automaticamente, niente
 *    da fare. Su /profile/<userId>/subscribed il flag enableQuickAdd
 *    e' off (vedi profile-issue-store v1.23).
 */
...
const useProfileIssueActions = () => {
  ...
  // PATCH v1.23: quickAddIssue per Your Work. Auto-assign su vista
  // 'assigned' (filtro server-side assignees=userId): senza questo
  // i task creati sparirebbero dalla lista al refresh.
  const quickAddIssue = useCallback(
    async (projectId: string | undefined | null, data: TIssue) => {
      if (!workspaceSlug || !projectId) return;
      let payload = data;
      if (issues.currentView === "assigned" && userId) {
        const existing = (data.assignee_ids ?? []) as string[];
        if (!existing.includes(userId)) {
          payload = { ...data, assignee_ids: [...existing, userId] };
        }
      }
      return await issues.quickAddIssue(workspaceSlug, projectId, payload);
    },
    [issues.quickAddIssue, issues.currentView, workspaceSlug, userId]
  );
  ...
  return useMemo(
    ...
    [fetchIssues, createIssue, quickAddIssue, updateIssue, removeIssue, archiveIssue, updateFilters]
  );
};
...
const useGlobalIssueActions = () => {
  ...
  // PATCH v1.23: quickAddIssue per workspace views.
  // Niente magic auto-fill: il filtro della view e' arbitrario (state,
  // priority, ...) e l'utente sceglie consapevolmente.
  const quickAddIssue = useCallback(
    async (projectId: string | undefined | null, data: TIssue) => {
      if (!workspaceSlug || !projectId) return;
      return await issues.quickAddIssue(workspaceSlug, projectId, data);
  ...
  return useMemo(
    ...
    [createIssue, quickAddIssue, updateIssue, removeIssue, updateFilters]
  );
};
