 ...
 *  Filtri getter pubblici aggiornati per escludere i progetti "fittizi"
 *  (is_hidden=true). Aggiunto getter dedicato workspaceHiddenProjectId per
 *  permettere al codice client di accedere all'ID del progetto fittizio.
 */
...
export class ProjectStore implements IProjectStore {
  ...
  get filteredProjectIds() {
    ...
    let workspaceProjects = Object.values(this.projectMap).filter(
      (p) =>
        ...
        // PATCH v1.22b: nasconde i progetti fittizi workspace-level.
        !p.is_hidden &&
  ...
  get workspaceProjectIds() {
    ...
    const workspaceProjects = Object.values(this.projectMap).filter(
      // PATCH v1.22b: nasconde i progetti fittizi workspace-level.
      (p) => p.workspace === workspaceDetails.id && !p.archived_at && !p.is_hidden
    );
  ...
  /**
   * PATCH v1.22b: ID del progetto fittizio "Workspace" per il workspace
   * corrente, se gia' presente nel projectMap. Usato dal modal Create work
   * item per pre-selezionare la voce "Workspace" e dalle viste per
   * marcare visivamente i task workspace-level.
   * Nota: se non e' nel projectMap, e' ancora da fetchare via
   * useWorkspaceProject() hook (chiama backend GET /workspace-project/).
   */
  get workspaceHiddenProjectId() {
    const workspaceDetails = this.rootStore.workspaceRoot.currentWorkspace;
    if (!workspaceDetails) return undefined;
    const hiddenProject = Object.values(this.projectMap).find(
      (p) => p.workspace === workspaceDetails.id && !p.archived_at && p.is_hidden
    );
  ...
  get archivedProjectIds() {
    ...
    const projectIds = projects
      .filter(
        // PATCH v1.22b: nasconde i progetti fittizi workspace-level.
        (project) => project.workspace === currentWorkspace.id && !!project.archived_at && !project.is_hidden
      )
