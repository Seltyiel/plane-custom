 ...
 *
 * PATCH (plane-custom):
 *  11 metodi nel file originale usavano `throw error?.response` (senza
 *  .data). Quando `error.response` e' undefined (network error, abort,
 *  timeout) questi metodi lanciavano `undefined`, causando il famigerato
 *  "Uncaught (in promise) undefined" in console. Corretti tutti con la
 *  forma standard usata dagli altri ~21 metodi: `throw error?.response?.data`.
 *
 *  Metodi corretti:
 *   - getWorkspace
 *   - workspaceMemberMe
