 ...
 * rejections su fetchIssuesWithExistingPagination. In updateGlobalView
 * la chiamata a fetchIssuesWithExistingPagination e' fire-and-forget
 * (non awaited): se la fetch precedente viene abortita workspace.service
 * trasforma AbortError in `throw undefined` e il browser emette
 * "Uncaught (in promise) undefined" causando il blank screen di React.
 * Aggiungiamo un .catch(swallowAbort) per rendere la rejection benigna.
 */
...
const swallowAbort = (e: unknown) => {
  ...
  console.warn("[plane-custom][global-view-store] fetch rejected:", e);
};
  ...
  async updateGlobalView(
    ...
    try {
      ...
      if (shouldSyncFilters && !isEqual(currentViewData?.rich_filters || {}, currentView?.rich_filters || {})) {
        ...
        // PATCH v1.08: .catch(swallowAbort) per non lasciare rejection
        // uncaught quando la fetch precedente viene abortita al layout
        // change. Il metodo e' fire-and-forget (ritorna Promise ma non
        // viene awaited) quindi senza .catch il browser emette
        // "Uncaught (in promise) undefined" e React cestina il sottoalbero.
