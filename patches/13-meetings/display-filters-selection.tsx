 ...
 *  Full-replacement per iniettare il toggle "Show meetings in calendar"
 *  dentro il Display dropdown, accanto a "Show sub-work items".
 *  Visibile SOLO se layout=calendar. Toggle gating: workspace admin only.
 *
 *  Differenza vs stock: 1 import + 1 conditional render JSX nella sezione
 *  Options (stessa div di FilterExtraOptions, no divider).
 */
...
// PATCH v1.34h-2: toggle Show meetings inline (custom).
import { MeetingsShowToggle } from "@/components/meetings/meetings-show-toggle";
...
export const DisplayFiltersSelection = observer(function DisplayFiltersSelection(props: Props) {
  ...
  // PATCH v1.34h-2: toggle Meetings appare solo per layout calendar.
  const showMeetingsToggle = displayFilters?.layout === "calendar";
    ...
    <div className="vertical-scrollbar relative scrollbar-sm h-full w-full divide-y divide-subtle-1 overflow-hidden overflow-y-auto px-2.5">
      ...
      {layoutDisplayFiltersOptions?.extra_options.access && (
        <div className="py-2">
          ...
          />
          {/* PATCH v1.34h-2: Meetings toggle in-section (solo Calendar). */}
          {showMeetingsToggle && <MeetingsShowToggle />}
        </div>
      )}

      {/* Fallback: extra_options.access false ma layout=calendar -> standalone. */}
      {!layoutDisplayFiltersOptions?.extra_options.access && showMeetingsToggle && (
        <div className="py-2">
          <MeetingsShowToggle />
        </div>
      )}
